...
def calculate_loss(noise_image, model, batch_idx, device, methode, lambda_inv=2):
...
def train(model, device, dataLoader, optimizer, mode, writer, epoch, methode):
    ...
    for batch_idx, (noise_images, clean, noise, std, amp, _, _, _) in enumerate(dataLoader):
        ...
        if mode == "train":
            model.train()
            loss, denoised, mask_orig = calculate_loss(noise_images, model, batch_idx, device, methode)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        else:
        ...
        lsd_log.append(1)#round(lsd.item(),3))
...
def main(argv=[]):
    ...
    for i in range(len(masking_methodes)):
        ...
        store_path = Path(os.path.join(store_path_root, f"n2same-{mask_methode}"))
        ...
        model_save_path = os.path.join(store_path, "models", f"last-model-n2same_{mask_methode}_syn.pth")
        torch.save(model.state_dict(), model_save_path)
